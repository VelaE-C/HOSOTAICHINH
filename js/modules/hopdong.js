  ...
  const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>${c.doc_number}</title>
    ...
    <body>
      ...
      <table>
        ...
        <tr><td colspan="4" style="text-align:right;font-weight:700">Hoàn thành duyệt</td><td>${vnDate(c.completed_at)}</td></tr>
      </table>
    </body></html>`;
...
async function openEditModal(c, user, onClose) {
  ...
  modal.querySelector('#btnSave').addEventListener('click', async () => {
    ...
    if (!project_id || !partner_id || !value) return toast('Điền đủ thông tin bắt buộc', 'error');
